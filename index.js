...
app.post('/todos', (req, res) => {
    ...
    db.run("INSERT INTO todos (task, is_done) VALUES (?,?)", [newTodo.task, newTodo.is_done], (err) => {
        if (err) {
            res.status(500).send(err);
        } else {
...
app.put('/todos/:id', (req, res) => {
    ...
    db.run("UPDATE todos SET task = ?, is_done = ? WHERE id = ?", [updatedTodo.task, updatedTodo.is_done, todoId], (err) => {
        if (err) {
            res.status(500).send(err);
        } else {
